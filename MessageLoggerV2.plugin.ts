...
// Using BdApi instead of Vendetta imports
const BdApiInstance = window.BdApi;
const { NavigationUtils, MessageActions } = BdApiInstance.Webpack.getByProps('transitionTo', 'deleteMessage');
...
class Injector {
  after(object: any, methodName: string, callback: Function): () => void {
    const original = object[methodName];
    object[methodName] = function(...args: any[]) {
      const result = original.apply(this, args);
      return callback.call(this, args, result);
    };
    return () => { object[methodName] = original; };
  }
  
  instead(object: any, methodName: string, callback: Function): () => void {
    const original = object[methodName];
    object[methodName] = function(...args: any[]) {
      return callback.call(this, args, original.bind(this));
    };
    return () => { object[methodName] = original; };
  }
}
